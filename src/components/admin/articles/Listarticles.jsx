...
const Listarticles = () => {
  ...
  const getProductsDispatch = useCallback(() => {
    console.time('getProductsDispatch');
    dispatch(getArticlesPagination());
    console.timeEnd('getProductsDispatch');
  ...
  useEffect(() => {
    getProductsDispatch();
  }, [page, limit, searchTerm, getProductsDispatch]);

  const handleLimitChange = useCallback((event) => {
    console.time('handleLimitChange');
    dispatch(setLimit(parseInt(event.target.value, 10)));
    dispatch(setPage(1)); // Réinitialiser la page lorsque le nombre d'éléments par page change
    console.timeEnd('handleLimitChange');
  }, [dispatch]);
